...
public:
    ...
    void format(const details::log_msg &msg, const std::tm &tm_time, memory_buf_t &dest) override
    {
        // NOTE: color-related stuff is defined in fmt/color.h

        if (msg.level != level::trace) {
            msg.color_range_start = dest.size();
            fmt_helper::append_string_view(level::to_string_view(msg.level), dest);
            msg.color_range_end = dest.size();

            // if (!msg.source.empty() && (msg.level == level::debug || msg.level == level::err || msg.level == level::critical)) {
            //     fmt_helper::append_string_view(" at ", dest);
...
SPDLOG_INLINE pattern_formatter::pattern_formatter(pattern_time_type time_type, std::string eol)
    : pattern_(SPDLOG_DEFAULT_PATTERN)
    , eol_(std::move(eol))
    , pattern_time_type_(time_type)
